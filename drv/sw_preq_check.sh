...
if [ "$IS_X_SERVER_RUNNING" != "" ]; then
	if [ "" = "$SEARCH_OUTPUT" ]; then
		...
		gstatus=1
		echo "Language packs are missing from your system, Install and apply language packs before continuing with the installation of the AMD Catalyst driver. To install and apply the language packs, go to System Settings -> Language Support, will install default packages, click Apply System-Wide button, and then restart your system" >> ${LOG_FILE}
	fi
...
if  [ "$uname_r_major" -eq 3 -a "$uname_r_minor" -gt 6 ]; then
	if [ ! -f /lib/modules/${uname_r}/build/include/generated/uapi/linux/version.h ]; then
		...
		echo "fglrx installation requires that the system has kernel headers for 3.7 release.  "/lib/modules/${uname_r}/build/include/generated/uapi/linux/version.h" cannot be found on this system." >> ${LOG_FILE}
...
if [ "$IS_X_SERVER_RUNNING" != "" ]; then
	...
	if [ $rstatus = "2" ]; then
		...
		ZENITY_BIN=`which zenity 2> /dev/null`
                     if [ `id -u` -eq 0 -a -n "${ZENITY_BIN}" -a -x "${ZENITY_BIN}" ]; then
                        ${ZENITY_BIN} --warning --text "Please install the required pre-requisites for package generation before proceeding with AMD Catalyst installation. Please check file usr/share/ati/fglrx-install.log for more details."
		     fi
	fi
		...
		ZENITY_BIN=`which zenity 2> /dev/null`
                     if [ `id -u` -eq 0 -a -n "${ZENITY_BIN}" -a -x "${ZENITY_BIN}" ]; then
                        ${ZENITY_BIN} --error --text "Please install the required pre-requisites before proceeding with AMD Catalyst installation. Please check file usr/share/ati/fglrx-install.log for more details."
		     fi
        fi
	...
	if [ $rstatus = "2" ]; then
			...
			echo "WARNING: Please install the required pre-requisites for package generation before proceeding with AMD Catalyst installation. Please check file usr/share/ati/fglrx-install.log for more details."
	fi

	if [ $rstatus = "1" ]; then
			pstatus=1
			#echo "Console is running"
			echo "ERROR: Please install the required pre-requisites before proceeding with AMD Catalyst installation. Please check file usr/share/ati/fglrx-install.log for more details."
	fi
